...
# Need to change this path so it match the path in user's computer
PATH_TO_MODEL_O_STL_FILES = "/home/ron/Documents/FinalProject/Ron_Avraham_FInal_Project-main/gym/envs/mujoco/assets/sawyer/"

# User inputs
user_motor_input = input("Please choose the type of the motors - enter 'p' for position motor and 't' for torque motor")
while(user_motor_input not in MOTORS):
    user_motor_input = input("Please choose the type of the motors - enter 'p' for position motor and 't' for torque motor")
motor_type = "position" if user_motor_input == 'p' else "motor"

user_target_body_input = input("Please choose the type of the motors - enter 'b' for box 'c' for cylinder and 'e' for ellipsoid")
while(user_target_body_input not in TARGET_BODIES):
    user_target_body_input = input("Please choose the type of the motors - enter 'b' for box 'c' for cylinder and 'e' for ellipsoid")

output_file_name = input('Please choose a file name for the XML')
...
motor_lower_range = -10 if user_motor_input == 'p' else -0.035
motor_upper_range = 10 if user_motor_input == 'p' else 0.04
...
# Mass
proximal_part_mass = 0.013 # In kg
distal_part_mass = 0.03 # In kg

# PLA Density
pla_density = 1.4 #in g/cm3
pla_density = 1000 * pla_density #in kg/m3
...
if user_target_body_input == 'c':
    # Object to grab parameters - #cylinder
    diameter = 0.01
    height = 0.07
    target_body_volume = math.pi * height * diameter * diameter # in m3
    target_body_mass = pla_density * target_body_volume # in kg
    center_off_body_z = 0.21
    x_pos = 0.025
    y_pos = 0
    z_pos = center_off_body_z - height / 2
    target_body_input_line = f"""
    <geom mass="{target_body_mass}" pos="{x_pos} {y_pos} {z_pos}" size="{diameter} {height}" type="cylinder" name="target_body"/>
    """

elif user_target_body_input == 'b':
    # Object to grab parameters - #box
    width = 0.005
    length = 0.01
    height = 0.07
    target_body_volume = width * height * length
    target_body_mass = pla_density * target_body_volume
    center_off_body_z = 0.21
    x_pos = 0.018
    y_pos = 0
    z_pos = center_off_body_z - height / 2
    target_body_input_line = f"""
    <geom mass="{target_body_mass}" pos="{x_pos} {y_pos} {z_pos}" size="{width} {length} {height}" type="box" name="target_body"/>
    """

else:
    # Object to grab parameters - #elipsoid
    width = 0.005
    length = 0.01
    height = 0.07
    target_body_volume = 4/3 * math.pi * width * height * length
    target_body_mass = pla_density * target_body_volume
    center_off_body_z = 0.21
    x_pos = 0.019
    y_pos = 0
    z_pos = center_off_body_z - height / 2
    target_body_input_line = f"""
    <geom mass="{target_body_mass}" pos="{x_pos} {y_pos} {z_pos}" size="{width} {length} {height}" type="ellipsoid" name="target_body"/>
    """
...
<mujoco>
    ...
    <compiler meshdir="{PATH_TO_MODEL_O_STL_FILES}" angle="radian"/>
      ...
      <body name="base_link">
          ...
          <body name="swivel_r" pos="0.05 0.025 0.0625" euler="0 0 {30 * deg_to_rad}" >
              ...
              <body name="proximal_r" pos="0.0038 0 0.0475">
                  ...
                  <geom name="proximal_r" mass="{proximal_part_mass}" type="mesh" mesh="proximal_O" pos="-0.01 -0.008 0.045" euler="{90  * deg_to_rad} {180 * deg_to_rad} {-150 * deg_to_rad}"/>
                  ...
                  <body name="distal_r" pos="-0.027 0 0.068" euler="0 {65 * deg_to_rad} 0">
                      ...
                      <geom name="distal_r" mass="{distal_part_mass}" type="mesh" mesh="distal_O" pos="-0.015 0.015 0.015" euler="{-90 * deg_to_rad} {90 * deg_to_rad} 0"/>
          ...
          <body name="swivel_l" pos="0.05 -0.025 0.0625" euler="0 0 {-30 * deg_to_rad}">
              ...
              <body name="proximal_l" pos="0.0038 0 0.0475">
                  ...
                  <geom name="proximal_l" mass="{proximal_part_mass}" type="mesh" mesh="proximal_O" pos="-0.01 -0.008 0.045" euler="{90 * deg_to_rad} {180 * deg_to_rad} {-150 * deg_to_rad}"/>
                  ...
                  <body name="distal_l" pos="-0.027 0 0.068" euler="0 {65 * deg_to_rad} 0">
                      ...
                      <geom name="distal_l" mass="{distal_part_mass}" type="mesh" mesh="distal_O" pos="-0.015 0.015 0.015" euler="{-90 * deg_to_rad} {90 * deg_to_rad} 0"/>
          ...
          <body name="proximal_c" pos="-0.0138 0 0.1105">
              ...
              <geom name="proximal_c" mass="{proximal_part_mass}" type="mesh" mesh="proximal_O" pos="0 0.008 0.045" euler="{90 * deg_to_rad} 0 {-150 * deg_to_rad}"/>
              ...
              <body name="distal_c" pos="0.02 0 0.066" euler="0 {120 * deg_to_rad} 0">
                  ...
                  <geom name="distal_c" type="mesh" mass="{distal_part_mass}" mesh="distal_O" pos="-0.015 -0.015 -0.015" euler="{90 * deg_to_rad} {90 * deg_to_rad} 0"/>
   ...
   <actuator> <!-- Adding motors to the relevant joints -->
        <{motor_type} joint="joint_1" name="swivel_proximal_r_motor" gear="1" ctrllimited="true" ctrlrange="{motor_lower_range} {motor_upper_range}"/>

        <{motor_type} joint="joint_2" name="swivel_proximal_l_motor" gear="1" ctrllimited="true" ctrlrange="{motor_lower_range} {motor_upper_range}"/>

        <{motor_type} joint="joint_3" name="swivel_proximal_c_motor" gear="1" ctrllimited="true" ctrlrange="{motor_lower_range} {motor_upper_range}"/>
    </actuator>
...
with open(f"./gym/envs/mujoco/assets/{output_file_name}.xml", "w") as f:
    f.write(MODEL_XML)
